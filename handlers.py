...
# Путь к файлу для хранения зашифрованного пароля
PASSWORD_FILE = "data/admin_password.json"
# Пароль по умолчанию для Admin
DEFAULT_ADMIN_PASSWORD = "admin123"

# Инициализация шифрования
def initialize_encryption():
    """Инициализация ключа шифрования и сохранение пароля, если он ещё не существует"""
    ...
    if not os.path.exists(PASSWORD_FILE):
        ...
        # Сохраняем ключ и зашифрованный пароль
...
def update_admin_password(new_password):
    ...
    # Обновляем зашифрованный пароль в файле, сохраняя тот же ключ
    with open(PASSWORD_FILE, "r") as f:
        data = json.load(f)
    data["encrypted_password"] = base64.urlsafe_b64encode(encrypted_password).decode()
    with open(PASSWORD_FILE, "w") as f:
        json.dump(data, f)
...
def process_teacher_input(message: types.Message):
    ...
    for item in unique_entries:
        ...
        subject_str = f"{item['subject']}"[:min_width-2].ljust(min_width)
